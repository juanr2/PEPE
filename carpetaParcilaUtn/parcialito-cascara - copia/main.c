...
int main()
{
    ...
    char seguir='s';
    int opcion,i,j,k,indexFree,indexFreeProductProvider,auxIdProduct,auxQuantity,auxSelection,firstFlag=1,auxCodeProduct,foundIndexProduct,foundIndexProductProvider;
    ...
    setProduct(prod,1,2 ,"papa","limpia",8,15);

    //setProductProvider(prodProvider,indexFreeProductProvider,auxIdProduct ,auxSelection);
    //auxSelection es el id provider
    setProductProvider(prodProvider,0,2 ,2);
    setProductProvider(prodProvider,1,2 ,1);

    showProduct(prod,CANTPRODUCT);
     showProductProvider(prodProvider,CARDINAL);
    while(seguir=='s')
    {
        ...
        case 1:
            ...
            auxPrice=getFloat("INDIQUE EL PRECIO: ","ERROR:SOLO SE ACEPTAN NUMEROS","Rango valido [0 - 100]",1,100);
             ...
             showProvider(provider,CANTPROVIDER);
        ...
        case 4:
           ...
           for(i=0;i<CANTPROVIDER-1;i++){
            ...
            break;
                ...
                printf("idpro\tname\tiprod\n");

                for(i=0;i<CANTPROVIDER;i++){
                    for(j=0;j<CARDINAL;j++){
                        if(provider[i].idProvider==prodProvider[j].idProvider){

                            for(k=0;k<CANTPRODUCT;k++){
                                if(prodProvider[j].idProduct==prod[k].idProduct){

                                    printf("%d\t%s\t%d\n",provider[i].idProvider,provider[i].name,prodProvider[j].idProduct);
